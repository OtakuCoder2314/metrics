...
try {
  ...
  if (!/^[/-\w\d]+$/.test(branch)) { throw new Error(`invalid branch: ${branch}`) }
  if (!/^[-\w\d]+\/[-\w\d]+$/.test(repo)) { throw new Error(`invalid repo: ${repo}`) }
...
for (const path of fss.readdirSync(__presets)) {
  ...
  if (!(fss.lstatSync(paths.join(__presets, path))).isDirectory()) { continue }
    ...
    try {
      ...
      try {
        ...
        for (const [key, { type, required }] of Object.entries(properties)) {
          if (required) { test(`preset.${key} is defined`, () => expect(preset[key]).toBeDefined()) }
          test(`preset.${key} type is ${type}`, () => {
            switch (true) {
              ...
              case /\{\}$/.test(type): {
                ...
                if (typed !== 'unknown') {
                  for (const value of Object.values(preset[key])) { expect(typeof value).toBe(typed) }
                }
                return
              }
              case /\[\]$/.test(type): {
