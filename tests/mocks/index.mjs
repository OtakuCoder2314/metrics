    ...
    graphql = new Proxy(unmocked, {
      apply (target, that, args) {
        ...
        for (const mocked of Object.keys(mocks.github.graphql)) {
          if (new RegExp(`^query ${mocked.replace(/([.]\w)/g, (_, g) => g.toLocaleUpperCase().substring(1)).replace(/^(\w)/g, (_, g) => g.toLocaleUpperCase())} `).test(query)) { return mocks.github.graphql[mocked]({ faker, query, login }) }
        }
