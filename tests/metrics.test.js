  ...
  for (const name in metadata[type]) {
    const cases = yaml
      ...
      ?.map(({ name: test, with: inputs, modes = [], timeout }) => {
        ...
        if (!(metadata[type][name].supports?.includes('repository'))) { skip.add('repository') }
...
describe('GitHub Action', () =>
  describe.each([
    ...
    for (const [name, input, { skip = [], modes = [], timeout } = {}] of tests) {
      if ((skip.includes(template)) || ((modes.length) && (!modes.includes('action')))) { test.skip(name, () => null) } else { test(name, async () => expect(await action.run({ template, base: '', query: JSON.stringify(query), plugins_errors_fatal: true, dryrun: true, use_mocked_data: true, verify: true, retries: 1, ...input })).toBe(true), timeout) }
    }
  }))
...
describe('Web instance', () =>
  describe.each([
    ...
    for (const [name, input, { skip = [], modes = [], timeout } = {}] of tests) {
      if ((skip.includes(template)) || ((modes.length) && (!modes.includes('web')))) { test.skip(name, () => null) } else { test(name, async () => expect(await web.run({ template, base: 0, ...query, plugins_errors_fatal: true, verify: true, ...input })).toBe(true), timeout) }
    }
  }))
...
describe('Web instance (placeholder)', () =>
  describe.each([
    ...
    for (const [name, input, { skip = [], modes = [], timeout } = {}] of tests) {
      if ((skip.includes(template)) || ((modes.length) && (!modes.includes('placeholder')))) { test.skip(name, () => null) } else { test(name, async () => expect(await placeholder.run({ template, base: 0, ...query, ...input })).toBe(true), timeout) }
    }
  }))
