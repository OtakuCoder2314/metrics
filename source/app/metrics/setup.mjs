...
export default async function ({ log = true, sandbox = false, community = {}, extras = false } = {}) {
  ...
  // Load settings
  {
    ...
    try {
      ...
      if (sandbox) { logger('metrics/setup > load settings.json > skipped because in sandbox is enabled') } else {
        conf.settings = JSON.parse(`${await fs.promises.readFile(fd)}`)
        logger('metrics/setup > load settings.json > success')
      }
    } catch (error) {
      if (error?.code === 'ENOENT') { logger('metrics/setup > load settings.json > (missing)') } else { logger(`metrics/setup > load settings.json > (error : ${error})`) }
    } finally {
      await fd?.close()
    }
  }
    ...
    if ((Array.isArray(conf.settings.community.templates)) && (conf.settings.community.templates.length)) {
      ...
      for (const template of conf.settings.community.templates) {
        try {
          ...
          // JavaScript file
          if (trust) { logger(`metrics/setup > keeping @${name}/template.mjs (unsafe mode is enabled)`) } else if (fs.existsSync(path.join(__templates, `@${name}`, 'template.mjs'))) {
  ...
  if ((!conf.settings.outputs) || (!conf.settings.outputs.length)) { conf.settings.outputs = metadata.inputs.config_output.values } else { conf.settings.outputs = conf.settings.outputs.filter(format => metadata.inputs.config_output.values.includes(format)) }
