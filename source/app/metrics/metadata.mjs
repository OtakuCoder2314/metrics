...
export default async function metadata ({ log = true, diff = false } = {}) {
  ...
  for (const name of await fs.promises.readdir(__plugins)) {
    if (!(await fs.promises.lstat(path.join(__plugins, name))).isDirectory()) { continue }
    switch (name) {
      case 'community': {
        const ___plugins = path.join(__plugins, 'community')
        for (const name of await fs.promises.readdir(___plugins)) {
          if (!(await fs.promises.lstat(path.join(___plugins, name))).isDirectory()) { continue }
  ...
  for (const name of await fs.promises.readdir(__templates)) {
    if (!(await fs.promises.lstat(path.join(__templates, name))).isDirectory()) { continue }
...
metadata.plugin = async function ({ __plugins, __templates, name, logger }) {
  try {
    ...
    // Category
    if (!categories.includes(meta.category)) { meta.category = 'community' }
    if ((meta.category === 'github') && (!meta.disclaimer)) { meta.disclaimer = 'This plugin is not affiliated, associated, authorized, endorsed by, or in any way officially connected with [GitHub](https://github.com).\nAll product and company names are trademarks™ or registered® trademarks of their respective holders.' }
    ...
    // Inputs parser
    {
      meta.inputs = function ({ data: { user = null } = {}, q, account }, defaults = {}) {
        // Support check
        if (!account) { console.debug(`metrics/inputs > account type not set for plugin ${name}!`) }
        if (account !== 'bypass') {
          const context = q.repo ? 'repository' : account
          if (!meta.supports?.includes(context)) { throw { error: { message: `Unsupported context ${context}`, instance: new Error() } } }
        }
        // Special values replacer
        const replacer = value => {
          value = `${value}`.trim()
          if (user) {
            if (value === '.user.login') { return user.login ?? value }
            if (value === '.user.twitter') { return user.twitterUsername ?? value }
            if (value === '.user.website') { return user.websiteUrl ?? value }
          }
          return value
        }
        // Inputs checks
            ...
            (defaulted => {
              ...
              switch (type) {
                ...
                case 'array': {
                  ...
                  let parsed = []; let used = 'comma-separated'
                ...
                case 'string': {
                  ...
                  if ((Array.isArray(values)) && (!values.includes(value))) { return defaulted }
                  return value
                }
                // JSON
                case 'json': {
    ...
    // Enable state handler
    {
      meta.enabled = function (enabled, { extras = {}, error = true } = {}) {
        if ((process.env.GITHUB_ACTIONS) && (!enabled)) { console.warn(`::warning::Plugin "${name}" is currently disabled. Add "plugin_${name}: yes" to your workflow to enable it.`) }
        if ((error) && (!enabled)) { throw Object.assign(new Error(`Plugin "${name}" is disabled${process.env.GITHUB_ACTIONS ? '' : ' on this server'}`), { enabled: true }) }
    ...
    // Extra features parser
    {
      meta.extras = function (input, { extras = {}, error = true } = {}) {
        ...
        try {
          ...
          if (!Array.isArray(enabled)) { throw new Error(`metrics/extras > ${name} > ${key} > extras.features is not an array`) }
          if (extras.css) {
    ...
    // Action metadata
    {
      ...
      raw.split(/(?:\r?\n){2,}/m)
        ...
        .map(x => {
          ...
          if (input) { comments[input] = x.match(new RegExp(`(?<comment>[\\s\\S]*?)(?=(?:${Object.keys(inputs).sort((a, b) => b.length - a.length).join('|')}):)`))?.groups?.comment }
        })
        ...
        Object.entries(inputs).map(([key, { type, description: text, example, default: defaulted, min = 0, max = 9999, values, extras }]) => [
          ...
          (() => {
            switch (type) {
              ...
              case 'string': {
                if (Array.isArray(values)) { return { text, type: 'select', values, defaulted } }
                return { text, type: 'text', placeholder: example ?? defaulted, defaulted, extras }
              }
              case 'json':
                return { text, type: 'text', placeholder: example ?? defaulted, defaulted, extras }
    ...
    // Readme metadata
    {
      ...
      for (const template of await fs.promises.readdir(__templates)) {
        if (!(await fs.promises.lstat(path.join(__templates, template))).isDirectory()) { continue }
      ...
      const table = [
        ...
        Object.entries(inputs).map(([option, { description, type, ...o }]) => {
          const cell = []
          if (o.required) { cell.push('✔️ Required<br>') }
          if (type === 'token') { cell.push('🔐 Token<br>') }
          if (o.inherits) { cell.push(`⏩ Inherits <code>${o.inherits}</code><br>`) }
          if (o.global) { cell.push('⏭️ Global option<br>') }
          if (/^(?:[Ff]alse|[Oo]ff|[Nn]o|0)$/.test(o.preset)) { cell.push('⏯️ Cannot be preset<br>') }
          if (o.testing) { cell.push('🔧 For development<br>') }
          if (!Object.keys(previous?.inputs ?? {}).includes(option)) { cell.push('✨ On <code>master</code>/<code>main</code><br>') }
          if (o.extras) {
            cell.push('🌐 Web instances must configure <code>settings.json</code>:')
            cell.push('<ul>')
            for (const permission of o.extras) { cell.push(`<li><i>${permission}</i></li>`) }
            cell.push('</ul>')
          }
          cell.push(`<b>type:</b> <code>${type}</code>`)
          if ('format' in o) { cell.push(`<i>(${Array.isArray(o.format) ? o.format[0] : o.format})</i>`) }
          if ('min' in o) { cell.push(`<i>(${o.min} ≤`) }
          if (('min' in o) || ('max' in o)) { cell.push(`${'min' in o ? '' : '<i>('}𝑥${'max' in o ? '' : ')</i>'}`) }
          if ('max' in o) { cell.push(`≤ ${o.max})</i>`) }
          cell.push('<br>')
          if ('zero' in o) { cell.push(`<b>zero behaviour:</b> ${o.zero}</br>`) }
          if (('default' in o) && (o.default !== '')) {
            let text = o.default
            if (o.default === '.user.login') { text = '<code>→ User login</code>' }
            if (o.default === '.user.twitter') { text = '<code>→ User attached twitter</code>' }
            if (o.default === '.user.website') { text = '<code>→ User attached website</code>' }
            if (type === 'json') { text = `<details><summary>→ Click to expand</summary><pre language="json"><code>${text}</code></pre></details>` }
            cell.push(`<b>default:</b> ${text}<br>`)
          }
          if ('values' in o) { cell.push(`<b>allowed values:</b><ul>${o.values.map(value => `<li>${value}</li>`).join('')}</ul>`) }
          return `  <tr>
  ...
  try {
    ...
    return {
      ...
      check ({ q, account = 'bypass', format = null }) {
        ...
        if (account !== 'bypass') {
          const context = q.repo ? 'repository' : account
          if ((Array.isArray(this.supports)) && (!this.supports.includes(context))) { throw new Error(`template not supported for: ${context}`) }
        }
        // Format check
        if ((format) && (Array.isArray(this.formats)) && (!this.formats.includes(format))) { throw new Error(`template not supported for: ${format}`) }
      }
    }
  } catch (error) {
...
metadata.to = {
  ...
  yaml (key, { name = '' } = {}) {
    const parts = []
    if (key !== 'enabled') { parts.unshift(key.replace(/\./g, '_')) }
    if (name) { parts.unshift((name === 'base') ? name : `plugin_${name}`) }
    return parts.join('_')
  }
}
