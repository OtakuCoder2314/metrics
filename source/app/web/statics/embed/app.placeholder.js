(function (
  ...
  async function staticPlaceholder (condition, name) {
    if (!condition) return ''
    return await fetch(`/.placeholders/${name}`)
      .then((response) => response.text())
      .catch(() => '(could not render placeholder)')
  }
  ...
  globalThis.placeholder = async function (set) {
    ...
    await Promise.all(
      partials.map(
        async (partial) =>
          await load(
            `/.templates/${set.templates.selected}/partials/${escape(partial)}.ejs`
          )
      )
    )
    // Trap includes
    ...
    const data = {
      ...
      async $include (path) {
        const partial = await load(
          `/.templates/${set.templates.selected}/${escape(path)}`
        )
      ...
      user: {
        ...
        repositories: {
          ...
          totalDiskUsage: faker.number.int(100000),
          nodes: []
        },
      ...
      plugins: {
        ...
        ...(set.plugins.enabled.tweets
          ? {
              tweets: {
                ...
                profile: {
                  profile_image_url: faker.image.urlLoremFlickr({
                    category: 'people'
                  }),
                  name: '',
                  verified: false,
        ...
        ...(set.plugins.enabled.lines
          ? {
              lines: {
                ...
                sections: options['lines.sections']
                  .split(',')
                  .map((x) => x.trim()),
                repos: new Array(options['lines.repositories.limit'] || 4)
                  .fill(null)
                  .map((_) => ({
                ...
                history: await staticPlaceholder(
                  set.plugins.enabled.lines,
                  'lines.history.svg'
                )
              }
        ...
        ...(set.plugins.enabled.followup
          ? {
              followup: {
                sections: options['followup.sections']
                  .split(',')
                  .map((x) => x.trim())
                  .filter((x) => ['user', 'repositories'].includes(x)),
                issues: {
                  get count () {
                ...
                user: {
                  issues: {
                    get count () {
                      return (
                        this.open + this.closed + this.drafts + this.skipped
                      )
                    },
                    open: faker.number.int(1000),
                    closed: faker.number.int(1000),
                  ...
                  pr: {
                    get count () {
                      return (
                        this.open + this.closed + this.merged + this.drafts
                      )
                    },
                    open: faker.number.int(1000),
                    closed: faker.number.int(1000),
        ...
        ...(set.plugins.enabled.notable
          ? {
              notable: {
                contributions: new Array(2 + faker.number.int(2))
                  .fill(null)
                  .map((_) => ({
                    get name () {
                      return options['notable.repositories']
                        ? this.handle
                        : this.handle.split('/')[0]
                    },
          ...
          ? {
              achievements: {
                ...
                list: new Array(8)
                  ...
                  .filter(({ rank }) =>
                    options['achievements.secrets'] ? true : rank !== '$'
                  )
                  .filter(
                    ({ rank }) =>
                      ({ S: 5, A: 4, B: 3, C: 2, $: 1, X: 0 })[rank] >=
                      { S: 5, A: 4, B: 3, C: 2, $: 1, X: 0 }[
                        options['achievements.threshold']
                      ]
                  )
                  .sort(
                    (a, b) =>
                      ({ S: 5, A: 4, B: 3, C: 2, $: 1, X: 0 })[b.rank] +
                      b.progress * 0.99 -
                      ({ S: 5, A: 4, B: 3, C: 2, $: 1, X: 0 }[a.rank] +
                        a.progress * 0.99)
                  )
        ...
        ...(set.plugins.enabled.sponsors
          ? {
              sponsors: {
                sections: options['sponsors.sections']
                  .split(',')
                  .map((x) => x.trim()),
                about: 'A new way to contribute to open source',
                list: new Array(Number(faker.number.int(40)))
                  .fill(null)
                  .map((_) => ({
        ...
        ...(set.plugins.enabled.sponsorships
          ? {
              sponsorships: {
                sections: options['sponsorships.sections']
                  .split(',')
                  .map((x) => x.trim()),
                amount: faker.number.int(1000),
                list: new Array(2 + faker.number.int(8))
                  .fill(null)
                  .map((_) => ({
                    login: faker.internet.userName(),
                    avatar:
        ...
        ...(set.plugins.enabled.languages
          ? {
              languages: {
                ...
                sections: options['languages.sections']
                  .split(', ')
                  .map((x) => x.trim())
                  .filter((x) => /^(most-used|recently-used)$/.test(x)),
                ...
                'stats.recent': {
                  ...
                  get lines () {
                    return Object.fromEntries(
                      Object.entries(this.recent).map(([key, { value }]) => [
                        key,
                        value
                      ])
                    )
                ...
                favorites: distribution(options['languages.limit'] || 8).map(
                  (value, index, array) => ({
                    name:
                      index + 1 === array.length && options['languages.other']
                        ? 'Other'
                        : faker.lorem.word(),
                ...
                recent: distribution(options['languages.limit'] || 8).map(
                  (value, index, array) => ({
                    name:
                      index + 1 === array.length && options['languages.other']
                        ? 'Other'
                ...
                get verified () {
                  return options['languages.indepth']
                    ? { signature: faker.number.int(this.commits) }
                    : null
                },
        ...
        ...(set.plugins.enabled.rss
          ? {
              rss: {
                ...
                feed: new Array(Number(options['rss.limit']))
                  .fill(null)
                  .map((_) => ({
        ...
        ...(set.plugins.enabled.nightscout
          ? {
              nightscout: {
                url:
                  options['nightscout.url'] != null &&
                  options['nightscout.url'] != 'https://example.herokuapp.com'
                    ? options['nightscout.url']
          ...
          ? {
              pagespeed: {
                ...
                scores: [
                  ...
                  ...(options['pagespeed.pwa']
                    ? [{ score: faker.number.int({ max: 1 }), title: 'PWA' }]
                    : [])
                ],
                metrics: {
                ...
                list: new Array(Number(options['projects.limit']))
                  ...
                  .map((_) => ({
                    ...
                    ...(faker.datatype.boolean()
                      ? {
                          items: new Array(faker.number.int(4))
                            .fill(null)
                            .map(() => ({
        ...
        ...(set.plugins.enabled.discussions
          ? {
              discussions: {
                categories: {
                  stats: {
                    '🙏 Q&A': faker.number.int(100),
                    '📣 Announcements': faker.number.int(100),
                    '💡 Ideas': faker.number.int(100),
                    '💬 General': faker.number.int(100)
                  },
                  favorite: '📣 Announcements'
                },
                upvotes: {
                  discussions: faker.number.int(1000),
                  comments: faker.number.int(1000)
                },
                ...
                display: {
                  categories: options['discussions.categories']
                    ? {
                        limit:
                          options['discussions.categories.limit'] || Infinity
                      }
                    : null
                }
              }
        ...
        ...(set.plugins.enabled.starlists
          ? {
              starlists: {
                lists: new Array(Number(options['starlists.limit']))
                  .fill(null)
                  .map((_) => ({
                    ...
                    repositories: new Array(
                      Number(options['starlists.limit.repositories'])
                    )
                      .fill(null)
                      .map((_, i) => ({
                        description: !i
        ...
        ...(set.plugins.enabled.stargazers
          ? {
              __stargazers: {
                worldmap: await staticPlaceholder(
                  options['stargazers.worldmap'],
                  'stargazers.worldmap.svg'
                )
              },
              get stargazers () {
                const dates = []
                ...
                for (let d = -14; d <= 0; d++) {
                  const date = new Date(Date.now() - d * 24 * 60 * 60 * 1000)
                    ...
                    .substring(0, 10)
                  dates.push(date)
                  result.total.dates[date] =
                    total +=
                    result.increments.dates[date] =
        ...
        ...(set.plugins.enabled.wakatime
          ? {
              get wakatime () {
                const stats = (array) => {
                  const elements = []
                  const results = new Array(4 + faker.number.int(2))
                    .fill(null)
                    .map((_) => ({
                      name: array
                ...
                return {
                  ...
                  time: {
                    total: faker.number.int(100000),
                    daily: faker.number.int(24)
                  },
                  editors: stats([
                    'VS Code',
                    'Chrome',
              ...
              anilist: {
                ...
                get lists () {
                  const media = (type) => ({
                    ...
                    scores: {
                      user: faker.number.int(100),
                      community: faker.number.int(100)
                    },
        ...
        ...(set.plugins.enabled.steam
          ? {
              steam: {
                sections: options['steam.sections']
                  .split(',')
                  .map((x) => x.trim())
                  .filter((x) => x),
                player: {
        ...
        ...(set.plugins.enabled.leetcode
          ? {
              leetcode: {
                ...
                sections: options['leetcode.sections']
                  .split(',')
                  .map((x) => x.trim())
                  .filter((x) => x),
                ...
                skills: new Array(
                  Number(options['leetcode.limit.skills']) || 10
                )
                  .fill(null)
                  .map((_) => ({
                ...
                recent: new Array(Number(options['leetcode.limit.recent']) || 2)
                  .fill(null)
                  .map((_) => ({
                    title: faker.lorem.sentence(),
                    date: faker.date.recent()
                    ...
                    (_) =>
                      [
                        ...
                        },
                        {
                          ...
                          title: faker.lorem.sentence(),
                          lines: {
                            added: faker.number.int(1000),
                            deleted: faker.number.int(1000)
                          },
                        ...
                        },
                        {
                          ...
                          ref: {
                            ...
                            type: faker.helpers.arrayElement(['tag', 'branch'])
                          },
                          timestamp: faker.date.recent()
                        },
                        {
        ...
        ...(set.plugins.enabled.isocalendar
          ? {
              isocalendar: {
                streak: {
                  max: 30 + faker.number.int(20),
                  current: faker.number.int(30)
                },
                max: 10 + faker.number.int(40),
                average: faker.number.float(10),
                svg: await staticPlaceholder(
                  set.plugins.enabled.isocalendar,
                  `isocalendar.${options['isocalendar.duration']}.svg`
                ),
        ...
        ...(set.plugins.enabled.calendar
          ? {
              calendar: {
                years: new Array(options['calendar.years'] || 2)
                  .fill(0)
                  .map((_, index) => ({
        ...
        ...(set.plugins.enabled.support
          ? {
              support: {
                stats: {
                  solutions: faker.number.int(100),
                  posts: faker.number.int(1000),
                  topics: faker.number.int(1000),
                  received: faker.number.int(1000),
                  hearts: faker.number.int(1000)
                },
                badges: {
                  uniques: [],
                  multiples: [],
        ...
        ...(set.plugins.enabled.stackoverflow
          ? {
              stackoverflow: {
                sections: options['stackoverflow.sections']
                  .split(',')
                  .map((x) => x.trim())
                  .filter((x) => x),
                ...
                'answers-top': new Array(options['stackoverflow.limit'])
                  .fill(null)
                  .map((_) => ({
                    type: 'answer',
                ...
                'questions-top': new Array(options['stackoverflow.limit'])
                  .fill(null)
                  .map((_) => ({
                    type: 'question',
                    title: faker.lorem.sentence(),
