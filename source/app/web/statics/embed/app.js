  ...
  return new Vue({
    ...
    async mounted () {
      ...
      try {
        ...
        if (localStorage.getItem('session.metrics')) {
          axios.defaults.headers.common['x-metrics-session'] =
            localStorage.getItem('session.metrics')
        }
      } catch (error) {}
      // Init
      await Promise.all([
        ...
        (async () => {
          ...
          this.plugins.list = plugins.filter(
            ({ name }) =>
              metadata[name]?.supports.includes('user') ||
              metadata[name]?.supports.includes('organization')
          )
          const categories = [
            ...new Set(this.plugins.list.map(({ category }) => category))
          ]
          this.plugins.categories = Object.fromEntries(
            categories.map((category) => [
              category,
              this.plugins.list.filter((value) => category === value.category)
            ])
          )
        })(),
        // Base
        (async () => {
          ...
          this.plugins.enabled.base = Object.fromEntries(
            base.map((key) => [key, true])
          )
        })(),
        // Extras
        (async () => {
    ...
    data: {
      ...
      config: Object.fromEntries(
        Object.entries(metadata.core.web).map(([key, { defaulted }]) => [
          key,
          defaulted
        ])
      ),
      metadata: Object.fromEntries(
        Object.entries(metadata).map(([key, { web }]) => [key, web])
      ),
      hosted: null,
      extras: false,
      oauth: false,
    ...
    computed: {
      ...
      unusable () {
        const plugins = Object.entries(this.plugins.enabled)
          .filter(
            ([key, value]) =>
              value == true &&
              !this.plugins.list.filter(({ name }) => name === key)[0]?.enabled
          )
          .map(([key]) => key)
        const options = this.edited.filter(
          (option) => !this.supports(this.plugins.options.descriptions[option])
        )
        return [...plugins, ...options].sort()
      },
      // Edited plugins options
      edited () {
        return Object.keys(this.plugins.enabled).flatMap((plugin) =>
          Object.keys(this.options({ name: plugin })).filter(
            (key) =>
              this.plugins.options[key] !==
              metadata[plugin]?.web[key]?.defaulted
          )
        )
      },
      // User's avatar
      ...
      url () {
        ...
        // Base options
        const base = Object.entries(this.plugins.options)
          .filter(
            ([key, value]) =>
              key in metadata.base.web &&
              value !== metadata.base.web[key]?.defaulted
          )
          .map(([key, value]) => `${key}=${encodeURIComponent(value)}`)
        // Config
        const config = Object.entries(this.config)
          .filter(
            ([key, value]) =>
              value && value !== metadata.core.web[key]?.defaulted
          )
          .map(([key, value]) => `config.${key}=${encodeURIComponent(value)}`)
        // Template
        const template =
          this.templates.selected !== this.templates.list[0]
      ...
      scopes () {
        return new Set([
          ...Object.entries(this.plugins.enabled)
            ...
            .flatMap(([key]) => metadata[key].scopes),
          ...(Object.entries(this.plugins.enabled.base).filter(
            ([key, value]) => value
          ).length
            ? metadata.base.scopes
            : [])
        ])
      },
        ...
        return [
          ...
          ...(this.scopes.size
            ? [
                ...
                ...[...this.scopes].map(
                  (scope) =>
                    `          #  - ${scope}${scope === 'public_access' ? ' (default scope)' : ''}`
                ),
          ...
          ...[
            ...
            ...Object.entries(this.plugins.options)
              .filter(([key, value]) => value && !(key in metadata.base.web))
              .filter(([key, value]) => this.plugins.enabled[key.split('.')[0]])
              .map(
                ([key, value]) =>
            ...
            ...Object.entries(this.config)
              ...
              .map(
                ([key, value]) =>
                  `          config_${key.replace(/[.]/g, '_')}: ${typeof value === 'boolean' ? { true: 'yes', false: 'no' }[value] : value}`
              )
          ].sort()
        ].join('\n')
      },
      ...
      rlreset () {
        const reset = new Date(
          Math.max(this.requests.graphql.reset, this.requests.rest.reset)
        )
    ...
    methods: {
      ...
      async refresh () {
        ...
        if (keys) {
          for (const key of keys) this._computedWatchers[key]?.run()
          this.$forceUpdate()
        }
      },
      ...
      async generate () {
        ...
        try {
          await axios.get(
            `/.uncache?&token=${(await axios.get(`/.uncache?user=${this.user}`)).data.token}`
          )
      ...
      options ({ name }) {
        return Object.fromEntries(
          Object.entries(this.plugins.options.descriptions).filter(
            ([key]) =>
      ...
      supports (option) {
        ...
        if (!Array.isArray(this.extras)) return this.extras
        return (
          required.filter((permission) => !this.extras.includes(permission))
            .length === 0
        )
      }
    }
