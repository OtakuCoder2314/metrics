  ...
  return new Vue({
    ...
    async mounted () {
      ...
      try {
        ...
        if (localStorage.getItem('session.metrics')) { axios.defaults.headers.common['x-metrics-session'] = localStorage.getItem('session.metrics') }
      } catch (error) {}
      // Init
      await Promise.all([
    ...
    watch: {
      tab: {
        immediate: true,
        handler (current) {
          if (current === 'markdown') { this.clipboard = new ClipboardJS('.copy-markdown') } else if (current === 'action') { this.clipboard = new ClipboardJS('.copy-action') } else { this.clipboard?.destroy() }
        }
      },
      palette: {
        immediate: true,
