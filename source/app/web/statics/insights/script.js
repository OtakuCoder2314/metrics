(async function () {
  ...
  return new Vue({
    ...
    async mounted () {
      ...
      // From local storage
      this.localstorage = !!new URLSearchParams(location.search).get(
        'localstorage'
      )
      // User
    ...
    methods: {
      format (type, value, options) {
        switch (type) {
          case 'plural':
            if (options?.y) return value !== 1 ? 'ies' : 'y'
            return value !== 1 ? 's' : ''
          case 'number':
            return new Intl.NumberFormat(navigator.lang, options).format(value)
          case 'date':
            return new Intl.DateTimeFormat(navigator.lang, options).format(
              new Date(value)
            )
          case 'comment':
            ...
            return value
              .replace(
                RegExp(
                  baseUrl +
                    String.raw`(?:issues|pull|discussions)\/(\d+)(?:\?[\w-]+)?(#[\w-]+)?(?=<)`,
                  'g'
                ),
                (_, repo, id, comment) =>
              ...
              .replace(
                ...
                (_, repo, sha) =>
                  (options?.repo === repo ? '' : repo + '@') + sha
              ) // -> 'lowlighter/metrics@123abc'
              .replace(
                RegExp(
                  baseUrl + String.raw`compare\/([\w-.]+...[\w-.]+)(?=<)`,
                  'g'
                ),
                (_, repo, tags) =>
                  (options?.repo === repo ? '' : repo + '@') + tags
              ) // -> 'lowlighter/metrics@1.0...1.1'
              .replace(
                /[^&]#(\d+)/g,
                (_, id) =>
                  `<a href="https://github.com/${options?.repo}/issues/${id}">#${id}</a>`
              ) // -> #123
              .replace(
                /@([-\w]+)/g,
                (_, user) =>
        ...
        try {
          ...
          if (this.localstorage) {
            this.metrics = JSON.parse(
              localStorage.getItem('local.metrics') ?? 'null'
            )
            this.loaded = [
              'base',
              ...Object.keys(this.metrics?.rendered?.plugins ?? {})
            ]
            return
          }
          const { processing, ...data } = (
            await axios.get(`/insights/query/${this.user}`)
          ).data
          if (processing) {
            let completed = 0
            ...
            const retry = async (plugin, attempts = 60, interval = 10) => {
              if (this.loaded.includes(plugin)) return
              do {
                try {
                  const { data } = await axios.get(
                    `/insights/query/${this.user}/${plugin}`
                  )
                  if (!data) throw new Error(`${plugin}: no data`)
                  if (plugin === 'base') {
                    this.metrics = {
                      rendered: data,
                    ...
                    Object.assign(this.metrics.rendered.plugins, {
                      [plugin]: data
                    })
                  }
                  break
                  ...
                  console.warn(
                    `${plugin}: no data yet, retrying in ${interval} seconds`
                  )
                  await new Promise((solve) =>
                    setTimeout(solve, interval * 1000)
                  )
                }
              } while (--attempts)
              completed++
            ...
            await Promise.allSettled(
              data.plugins.map((plugin) => retry(plugin))
            )
          } else {
            this.metrics = data
            this.loaded = [
    ...
    computed: {
      ...
      warnings () {
        return Object.entries(this.metrics?.rendered.plugins ?? {})
      ...
      ranked () {
        return (
          this.metrics?.rendered.plugins?.achievements?.list
            ?.filter(({ leaderboard }) => leaderboard)
            .sort((a, b) =>
              a.leaderboard.type.localeCompare(b.leaderboard.type)
            ) ?? []
        )
      },
      achievements () {
        return (
          this.metrics?.rendered.plugins?.achievements?.list
            ...
            .filter(
              ({ title }) =>
                !/(?:automator|octonaut|infographile)/i.test(title)
            ) ?? []
        )
      },
      introduction () {
      ...
      languages () {
        return Object.assign(
          this.metrics?.rendered.plugins?.languages?.favorites ?? [],
          { total: this.metrics?.rendered.plugins?.languages.total }
        )
      },
      reactions () {
      ...
      stars () {
        return {
          repositories:
            this.metrics?.rendered.plugins?.stars?.repositories.map(
              ({ node, starredAt }) => ({ ...node, starredAt })
            ) ?? []
        }
      },
      topics () {
      ...
      account () {
        ...
        return {
          login,
          name,
          avatar: this.metrics?.rendered.computed.avatar,
          type: this.metrics?.rendered.account
        }
      },
      url () {
