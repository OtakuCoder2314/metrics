...
export default async function ({ login, q, imports, data, account }, { enabled = false, extras = false } = {}) {
  ...
  try {
    ...
    if ((!q.skyline) || (!imports.metadata.plugins.skyline.enabled(enabled, { extras }))) { return null }
    ...
    // Load page
    if (!url) { throw { error: { message: 'Skyline URL is not set' } } }
    ...
    if (ready) { await page.waitForFunction(ready.replaceAll('${login}', login).replaceAll('${year}', year), { timeout: 90 * 1000 }) }
    if ((wait) && (wait > 0)) { await new Promise(solve => setTimeout(solve, wait * 1000)) }
    if (hide) { await frame.evaluate(hide => [...document.querySelectorAll(hide)].map(element => element.style.display = 'none'), hide) }
