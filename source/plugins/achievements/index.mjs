...
function rank (x, [c, b, a, s, m]) {
  if (x >= s) { return { rank: 'S', progress: (x - s) / (m - s) } }
  if (x >= a) { return { rank: 'A', progress: (x - a) / (s - a) } } else if (x >= b) { return { rank: 'B', progress: (x - b) / (a - b) } } else if (x >= c) { return { rank: 'C', progress: (x - c) / (b - c) } }
    ...
    total.promise = new Promise(async (solve, reject) => {
      for (const method of ['graphql', 'browser']) {
        ...
        try {
          ...
          // Check setup state
          if ((!total.users) || (!total.repositories)) { throw new Error('Uncomplete setup') }
        } catch (error) {
          console.debug(`metrics/compute/plugins > achievements > setup error > ${error}`)
          continue
        }
      }
      if ((!total.users) || (!total.repositories)) { return reject('Failed to initiate total for achievement plugin') }
