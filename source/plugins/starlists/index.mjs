...
export default async function ({ login, q, imports, data, account }, { enabled = false, extras = false } = {}) {
  ...
  try {
    ...
    if ((!q.starlists) || (!imports.metadata.plugins.starlists.enabled(enabled, { extras }))) { return null }
    ...
    for (const list of lists) {
      ...
      if (_shuffle) { list.repositories = imports.shuffle(list.repositories) }
      ...
      if (languages) {
        ...
        for (const { language: { name, color } } of repositories) {
          let lang = name
          if (lang && lang.toLocaleLowerCase() in _languages_aliases) { lang = _languages_aliases[name.toLocaleLowerCase()] }
          if (lang) { list.languages[lang] = (list.languages[lang] ?? 0) + 1 }
          if (color) { colors[lang] = color }
        }
        ...
        for (let i = 1; i < list.languages.length; i++) { list.languages[i].x = (list.languages[i - 1]?.x ?? 0) + (list.languages[i - 1]?.value ?? 0) / visible }
      }
