...
export default async function ({ login, q, imports, graphql, queries, data, account }, { enabled = false, extras = false } = {}) {
  ...
  try {
    ...
    if ((!q.discussions) || (!imports.metadata.plugins.discussions.enabled(enabled, { extras }))) { return null }
    ...
    // Load started discussions
    {
      ...
      for (const category of [...fetched.map(({ category: { emoji, name } }) => `${imports.emoji.get(emoji) ?? emoji} ${name}`)]) { categories[category] = (categories[category] ?? 0) + 1 }
