...
export default async function ({ login, q, imports, data, graphql, queries, account }, { enabled = false, extras = false } = {}) {
  ...
  try {
    ...
    for (const reaction of reactions) { list[reaction] = (list[reaction] ?? 0) + 1 }
    const max = Math.max(...Object.values(list))
    for (const [key, value] of Object.entries(list)) { list[key] = { value, percentage: value / reactions.length, score: value / (display === 'relative' ? max : reactions.length) } }
