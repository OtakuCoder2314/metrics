...
export default async function ({ login, data, rest, q, account, imports }, { enabled = false, markdown = 'inline', extras = false } = {}) {
  ...
  try {
    ...
    if ((!q.activity) || (!imports.metadata.plugins.activity.enabled(enabled, { extras }))) { return null }
        ...
        .map(async ({ type, payload, actor: { login: actor }, repo: { name: repo }, created_at }) => {
          ...
          switch (type) {
            ...
            case 'PushEvent': {
              ...
              if (commits.slice(-1).pop()?.message.startsWith('Merge branch ')) { commits = commits.slice(-1) }
