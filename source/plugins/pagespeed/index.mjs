...
export default async function ({ login, imports, data, q, account }, { enabled = false, token = null, extras = false } = {}) {
  ...
  try {
    ...
    if ((!q.pagespeed) || (!imports.metadata.plugins.pagespeed.enabled(enabled, { extras }))) { return null }

    // Load inputs
    let { detailed, screenshot, url, pwa } = imports.metadata.plugins.pagespeed.inputs({ data, account, q })
    if (!imports.metadata.plugins.pagespeed.extras('token', { extras, error: false })) { token = '' }

    // Format url if needed
    if (!url) { throw { error: { message: 'Website URL is not set' } } }
    ...
    if (pwa) { categories.push('pwa') }
    let categories_required = ''
    for (const category of categories) { categories_required += `&category=${category}` }
    ...
    // Store screenshot
    if (screenshot) { result.screenshot = request.data.lighthouseResult.audits['final-screenshot'].details.data }
