  ...
  try {
    ...
    await Promise.all(tweets.map(async tweet => {
      ...
      if (attachments) {
        ...
        // Medias
        if (tweet.attachments) { tweet.attachments = await Promise.all(tweet.attachments.media_keys.filter(key => medias.get(key)).map(key => medias.get(key)).map(async url => ({ image: await imports.imgb64(url, { height: -1, width: 450 }) }))) }
        if (linked) {
          try {
            ...
            if (image) {
              if (tweet.attachments) { tweet.attachments.unshift([{ image, title, description, website }]) } else { tweet.attachments = [{ image, title, description, website }] }
            } else {
