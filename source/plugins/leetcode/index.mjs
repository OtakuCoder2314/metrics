...
export default async function ({ login, q, imports, data, queries, account }, { enabled = false, extras = false } = {}) {
  ...
  try {
    ...
    // Skills stats
    {
      ...
      for (const category in skills) { result.skills.push(...skills[category].map(({ tagName: name, problemsSolved: solved }) => ({ name, solved, category }))) }
    ...
    // Problems
    {
      ...
      for (const { difficulty, count } of all) { result.problems[difficulty] = { count, solved: 0 } }
      for (const { difficulty, count: solved } of submissions) { result.problems[difficulty].solved = solved }
    }
