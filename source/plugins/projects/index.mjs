...
export default async function ({ login, data, imports, graphql, q, queries, account }, { enabled = false, extras = false } = {}) {
  ...
  try {
    ...
    if ((!q.projects) || (!imports.metadata.plugins.projects.enabled(enabled, { extras }))) { return null }
    ...
    for (const identifier of repositories) {
      ...
      if (!project) { throw new Error(`Could not load project ${user}/${repository}`) }
    ...
    for (const project of projects.nodes) {
      ...
      if (time < 1) { updated = 'less than 1 day ago' } else if (time < 30) { updated = `${Math.floor(time)} day${time >= 2 ? 's' : ''} ago` }
      // Format progress
  ...
  // Handle errors
    ...
    throw imports.format.error(error, {
      descriptions: {
        custom (error) {
          if (error.errors?.map(({ type }) => type)?.includes('INSUFFICIENT_SCOPES')) { return 'Insufficient token scopes' }
          return null
        }
      }
    })
