...
export default async function ({ login, data, imports, q, rest, account }, { enabled = false, extras = false } = {}) {
  ...
  try {
    ...
    if ((!q.languages) || (!imports.metadata.plugins.languages.enabled(enabled, { extras }))) { return null }
    ...
    if (!indepth) { details = details.filter(detail => !['lines'].includes(detail)) }
    ...
    if ((`${colors}` in colorsets) && (limit <= 8)) { colors = colorsets[`${colors}`] }
    ...
    for (const repository of data.user.repositories.nodes) {
      ...
      if (!imports.filters.repo(repository, skipped)) { continue }
      ...
      for (const { size, node: { color, name } } of Object.values(repository.languages.edges)) {
        ...
        if (colors[name.toLocaleLowerCase()]) { customColors[name] = colors[name.toLocaleLowerCase()] }
        if (!languages.colors[name]) { languages.colors[name] = color }
    ...
    for (const { section, stats = {}, lines = {}, missed = { bytes: 0 }, total = 0 } of [{ section: 'favorites', stats: languages.stats, lines: languages.lines, total: languages.total, missed: languages.missed }, { section: 'recent', ...languages['stats.recent'] }]) {
      ...
      for (let i = 0; i < languages[section].length; i++) {
        ...
        if ((colors[i]) && (!colors[name.toLocaleLowerCase()])) { languages[section][i].color = colors[i] } else { languages[section][i].color = customColors[name] ?? languages.colors[name] ?? '#ededed' }
      }
    }
