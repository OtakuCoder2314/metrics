...
export default async function ({ login, q, imports, data, rest, account }, { enabled = false, extras = false } = {}) {
  ...
  try {
    ...
    if ((!q.code) || (!imports.metadata.plugins.code.enabled(enabled, { extras }))) { return null }
    ...
    for (const file of files) { file.language = await imports.language({ ...file, prefix: login }).catch(() => 'unknown') }
    ...
    if (snippet) {
      ...
      if (!snippet.patch.split('\n').shift().endsWith('@@')) { snippet.patch = snippet.patch.replace(/^(?<coord>@@.*?@@).*/, '$<coord>') }
