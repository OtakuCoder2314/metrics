...
export default async function ({ login, q, imports, data, account }, { token, enabled = false, extras = false } = {}) {
  ...
  try {
    ...
    const { data: { response: { game_count: count, games } } } = await imports.axios.get(urls.games.owned)
    ...
    if (sections.includes('player')) {
      ...
      const { data: { response: { players: [info] } } } = await imports.axios.get(urls.player.summary)
