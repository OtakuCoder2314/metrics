...
console.log(
  [
    ...
    "",
    cyan(
      `This script will help you to create ${italic("Nintendo API")} token that can be used to access data from ${italic("Nintendo Switch Online")} such as ${italic("Splatnet 3")}. While these tokens are read-only and cannot perform any action on your behave, their usage is not explicitely allowed by ${italic("Nintendo")}`,
    ),
    "",
  ].join("\n"),
);
const agreement = prompt(
  "I understand the risks and I understand that I will not be able to held the script authors as responsible for any damage caused by the usage of this script\nAgree? [y/n]",
);
if (agreement !== "y") Deno.exit(1);
...
try {
  ...
  console.log(
    [
      ...
      green(
        `Complete your integration with ${italic("metrics")} by performing the following steps:`,
      ),
      "",
      bgWhite(
        black(
          `1. Create a new secret ${cyan("SPLATOON_TOKEN")} in your repository and set its value to:`,
        ),
      ),
      "",
