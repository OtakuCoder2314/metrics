...
export default async function ({ login, q, imports, data, account }, { enabled = false, extras = false, token, 'statink.token': _statink_token } = {}) {
  ...
  try {
    ...
    if ((!q.splatoon) || (!imports.metadata.plugins.splatoon.enabled(enabled, { extras }))) { return null }
    ...
    // Save profile
    {
      ...
      if (!parsed?.loginState?.sessionToken) { throw new Error('Configuration is missing sessionToken') }
      if (statink) {
    ...
    if (!summary) { throw new Error('Failed to fetch player summary!') }
