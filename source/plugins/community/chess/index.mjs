...
export default async function ({ login, q, imports, data, account }, { enabled = false, token = '', extras = false } = {}) {
  ...
  try {
    ...
    if ((!q.chess) || (!imports.metadata.plugins.chess.enabled(enabled, { extras }))) { return null }
    ...
    for (const [key, defaulted] of Object.entries({ size: 40, delay: 1, duration: 4 })) {
      if (Number.isNaN(Number(animation[key]))) { animation[key] = defaulted }
      if (animation[key] < 0) { animation[key] = defaulted }
    }
